...
def convert_to_vr(infile, outfile):
    cmd = [
        "ffmpeg", "-i", infile, "-c:v", "libx264", "-preset", "slow", "-crf", "18",
        "-vf", "scale=3840x2160", "-c:a", "aac", "-b:a", "256k", outfile
    ]
    ...
    for file in sorted(files, key=str.casefold):
        ...
        if ".mp4" in file[-4:]:
            ...
            if screen_type == "screen":
                ...
                # Convert video to VR-compatible format
                outfile = os.path.splitext(fullfile)[0] + "_vr.mp4"
                convert_to_vr(fullfile, outfile)

                # Update entry with new VR-compatible file
                out_files[subDir]["list"].append(
                    {"name": title,
                     "src": urllib.parse.quote(outfile),
                     "thumbnail": urllib.parse.quote(img),
                     "screen_type": screen_type,
                     "date": datetime.fromtimestamp(date).strftime('%Y-%m-%d %H:%M:%S'),
                     "epoch": date
